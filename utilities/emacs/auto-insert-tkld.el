...
;;; An automatic file template package for Emacs.|
;;; $Date$|$Revision$|~/packages/auto-insert.tar.Z|
...
;;; $Log$
;;; Revision 1.1  1999-01-05 15:43:32  gjb
;;; Added -- for use with
;;; utilities/dev/new-{scheme-primitive,type-skeleton}.cc
;;;
;;; Revision 2.1  1997/03/28 19:07:50  gjb
;;; Added
;;;
...
(defconst auto-insert-version (substring "$Revision$" 11 -2)
  "$Id$
	  ...
	  (while (not (eobp))
		...
		(progn
		  ...
		  (cond
		   ...
		   ((looking-at "h")
		    (auto-insert-substitute (system-name)))
		   ((looking-at "a")
		    (auto-insert-substitute mail-self-address))
		   ((looking-at "u")
		    (auto-insert-substitute (user-login-name)))
		   ((looking-at "U")
		    (delete-char 1)
		   ...
		   ((looking-at "\\[\\(.*\\)%\\]")
		    (let* ((start (match-beginning 1)) (end (match-end 1))
		      ...
		      (delete-region (1- start) (+ end 1)) ; Why 1 ??
...
Matches the visited file name against the elements of `auto-insert-alist'."
  ...
  (if (or (equal t auto-insert-automatically)
	...
	;; find first filename matching alist entry
	...
	(if insert-file
	    (let* ((dirs (if auto-insert-search-current-dir
			   ...
			   auto-insert-path))
		   (file (concat (car dirs)
				 (if (string= "/"
					      (substring (car dirs) -1 nil))
				     ""
				   "/") insert-file)))
...
;; Misc functions that ought to be part of Emacs...
...
;; Make this the last in the list, so rcs-sk can check out files that
;; really should exist already.
(or (memq 'insert-auto-insert-files find-file-not-found-hooks)
    (setq find-file-not-found-hooks
	  (append find-file-not-found-hooks '(insert-auto-insert-files))))
