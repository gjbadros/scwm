...
(define (animate-iconify-or-deiconify window ms-delay 
				      before-animation-proc after-animation-proc pct-sizes)
  (let* ((wsize (window-frame-size window))
	 (wpos (point-list->point-pair (window-position window)))
	 (w (car wsize))
	 (h (cadr wsize)))
...
(define*-public (animate-iconify #&optional (win (get-window)))
   (animate-iconify-or-deiconify win 20 (lambda () (iconify win)) #f
						'(1.0 .9 .8 .7 .6 .5 .4 .3 .2 .1)))


(define*-public (animate-deiconify #&optional (win (get-window)))
   (animate-iconify-or-deiconify win 20 #f (lambda () (deiconify win))
