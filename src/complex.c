 ...
 ****************************************************************************/
void 
ComplexFunction(XEvent * eventp, Window w, ScwmWindow * tmp_win,
		unsigned long context, char *action, int *Module)
{
  ...
  if (mr == NULL) {
    if (!desperate)
      scwm_msg(ERR, "ComplexFunction", "No such function %s", action);
    return;
  }
  desperate = 0;
...
#if MS_DELETION_COMMENT
char *
expand(char *input, char *arguments[], ScwmWindow * tmp_win)
{
  int l, i, l2, n, k, j;
  char *out;
