...
ScwmWindow *Tmp_win;		/* the current scwm window */
 ...
 *
 ************************************************************************/
int GetContext(ScwmWindow *t, XEvent *e, Window *w)
{
  int Context,i;
...
void HandleConfigureRequest()
{
  ...
  if (XFindContext (dpy, cre->window, ScwmContext, (caddr_t *) &Tmp_win) ==
      XCNOENT)
    Tmp_win = NULL;
