...
/***********************************************************************
 *
 * code for launching scwm modules.
 *
 ***********************************************************************/
...
void 
executeModule(XEvent * eventp, Window w, ScwmWindow * tmp_win,
	      unsigned long context, char *action, int *Module)
{
  int scwm_to_app[2], app_to_scwm[2];
  int i, val, nargs = 0;
  char *cptr;
  char *args[20];
  char *arg1 = NULL;
  ...
  *Module = 0;
  ...
  while ((i < npipes) && (writePipes[i] >= 0))
    i++;
  if (i >= npipes) {
    scwm_msg(ERR, "executeModule", "Too many Accessories!");
    if (cptr != NULL)
      free(cptr);
    return;
  }
  ...
  if (pipe(scwm_to_app) != 0) {
    scwm_msg(ERR, "executeModule", "Failed to open pipe");
    return;
  }
  if (pipe(app_to_scwm) != 0) {
    scwm_msg(ERR, "executeModule", "Failed to open pipe2");
    if (cptr != NULL)
      free(cptr);
    close(scwm_to_app[0]);
    close(scwm_to_app[1]);
  ...
  sprintf(arg2, "%d", app_to_scwm[1]);
  sprintf(arg3, "%d", scwm_to_app[0]);
  ...
  val = fork();
  if (val > 0) {
    /* This fork remains running scwm */
    /* close appropriate descriptors from each pipe so
     * that scwm will be able to tell when the app dies */
    close(app_to_scwm[1]);
    close(scwm_to_app[0]);

    /* add these pipes to scwm's active pipe list */
    writePipes[i] = scwm_to_app[1];
    readPipes[i] = app_to_scwm[0];
    pipeOn[i] = -1;
    PipeMask[i] = MAX_MASK;
    free(arg1);
    pipeQueue[i] = NULL;
    ...
    /* make the PositiveWrite pipe non-blocking. Don't want to jam up
       scwm because of an uncooperative module */
#ifdef O_NONBLOCK
    ...
    if (fcntl(readPipes[i], F_SETFD, 1) == -1)
      scwm_msg(ERR, "executeModule", "module close-on-exec failed");
    ...
    execvp(arg1, args);
    scwm_msg(ERR, "executeModule", "Execution of module failed: %s", arg1);
    perror("");
    close(app_to_scwm[1]);
    close(scwm_to_app[0]);
    exit(1);
  } else {
    scwm_msg(ERR, "executeModule", "Fork failed");
    free(arg1);
  }
  return;
}
...
HandleModuleInput(Window w, int channel)
{
   ...
   * Now read an scwm bultin command line */
  ...
  if (size > 255) {
    scwm_msg(ERR, "HandleModuleInput",
...
void 
SendConfig(int module, unsigned long event_type, ScwmWindow * t)
{
 ...
 */
void 
SendStrToModule(XEvent * eventp, Window junk, ScwmWindow * tmp_win,
		unsigned long context, char *action, int *Module)
{
  char *module, *str;
  int i;
...
void 
send_list_func(XEvent * eventp, Window w, ScwmWindow * tmp_win,
...
void 
set_mask_function(XEvent * eventp, Window w, ScwmWindow * tmp_win,
		  unsigned long context, char *action, int *Module)
{
  int n, val1_unit;
  long val1;
